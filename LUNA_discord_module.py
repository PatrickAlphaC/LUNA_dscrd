...
loader = DirectoryLoader("Updraft-Converted-Docs", use_multithreading=True, loader_cls=TextLoader)
codehawks_loader = DirectoryLoader("Codehawks-Docs-Converted", use_multithreading=True, loader_cls=TextLoader)
data = loader.load()
...
vectordb = Chroma.from_documents(documents=texts, embedding=embeddings, persist_directory=persist_directory_updraft)
vectordb.persist()
docs_vectordb = Chroma.from_documents(documents=codehawks_texts, embedding=embeddings, persist_directory=persist_directory_docs)
docs_vectordb.persist()
docs_vectordb = None
vectordb = None
docs_vectordb = Chroma(persist_directory=persist_directory_docs, embedding_function=embeddings)
vectordb = Chroma(persist_directory=persist_directory_updraft, embedding_function=embeddings)
retriever = vectordb.as_retriever(search_type="similarity", search_kwargs={"k": 25})
docs_retriever = docs_vectordb.as_retriever(search_type="similarity", search_kwargs={"k": 25})

qa_chain = RetrievalQA.from_chain_type(llm=OpenAI(), chain_type="stuff", retriever=retriever, return_source_documents=True)
docs_chain = RetrievalQA.from_chain_type(llm=OpenAI(), chain_type="stuff", retriever=docs_retriever, return_source_documents=True)
        ...
        prompt_messages = [
            {
                "role": "user",
                "content": [
                    {"type": "text", "text": "Extract the text from this image. Only return the text from the image, format any code."},
...
def chat(input, ctx):
    ...
    current_user = ctx.author.name  # Assuming ctx.author.name is the correct user identifier
    ...
    if conversation_index is not None:
        ...
        data['conversations'][conversation_index]['conversation'].append({"role": "user", "content": input})
        ...
        conversation = [{"role": "system", "content": system_message}, {"role": "user", "content": input}]
        data['conversations'].append({"USER": current_user, "conversation": conversation})

    response = ""
    for part in Client().chat(model='dolphin-mixtral:latest', messages=conversation, stream=True):
        try:
    ...
    if conversation_index is not None:
        data['conversations'][conversation_index]['conversation'].append({"role": "assistant", "content": response})
    else:
        data['conversations'][-1]['conversation'].append({"role": "assistant", "content": response})
    ...
    if conversation_index is not None:
        ...
        if len(user_conversation) > 10:
            ...
            data['conversations'][conversation_index]['conversation'] = [first_message] + last_four_messages
    else:
...
async def add_answer(ctx):
    ...
    answer_image_content = analyze_images(answer_images) if len(answer_images) > 0 else []
...
@client.command()
async def note(ctx):
    arg_dict = {'f':find_question(ctx),
                'a':add_answer(ctx),
                'r':request_answer(ctx)}
    index_list = []
    for i in range(len(ctx.message.content)-1):
        if ctx.message.content[i] == '-':
            arg = ctx.message.content[i+1]
    ...
    if ctx.author.name in whitelist_users:
        ...
        try:
            ...
            for item in data:
                ...
                extracted_data.append({'name': name, 'reward': reward, 'description': description, 'status': status})
...
async def on_message(message):
    ...
    if message.author.name == 'mrpotatomagic':
        ...
        if message.author.name not in last_emoji_time or (current_time - last_emoji_time[message.author.name]).total_seconds() > 120:
